...
async function runEval() {
  const dataPath = path.join(__dirname, 'data/publications/2026/07/2026-07-25.json');
  ...
  const articles = [
    ...
    data.candidates.find(c => c.title.includes('카카오') || c.title.includes('파업')),
    data.candidates.find(c => c.fullText && c.title.includes('마이크로소프트'))
  ].filter(Boolean);
